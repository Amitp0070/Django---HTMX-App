...
def test_total_value_appear_on_list_page(user_transactions, client):
...
def test_category_filter(user_transactions, client):
    ...
    # get first two cetagories' primaryKeys
...
@pytest.mark.django_db
def test_cannot_add_transaction_with_nagetive_amount(
    user,
    transaction_dict_params,
    client
):
...
def test_update_transaction_request(user, transaction_dict_params, client):
    ...
    # check the request has Updated, not create a new transaction
    assert Transaction.objects.filter(user=user).count() == 1
    transaction = Transaction.objects.first()
