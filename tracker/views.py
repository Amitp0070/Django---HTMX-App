...
def transactions_list(request):
    ...
    transaction_page = paginator.page(1) # default to 1 when this view in triggered
...
def create_transaction(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            transaction.save()
            context = {'message': "Transaction was added succssefully!"}
...
def update_transaction(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            transaction.save()
            context = {'message': "Transaction was updated succssefully!"}
            return render(request, 'tracker/partials/transaction-success.html', context)
        else:
            context = {
                'form': form,
...
def delete_transaction(request, pk):
    ...
    transaction.delete()
    context = {
        'message': f"Transaction of {transaction.amount} on {transaction.date} was delete successfully!" 
    }
    return render(request, 'tracker/partials/transaction-success.html', context)
